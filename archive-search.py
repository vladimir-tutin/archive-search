...
@st.cache_data(ttl=3600)
def search_archive_with_duckduckgo(search_term, media_type, start_year=None, max_results=30, max_retries=5):
...
def filter_results_by_file_types(results, file_types_str):
    ...
    # Use ThreadPoolExecutor to parallelize file type filtering
    def has_desired_files(result):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        filtered = list(executor.map(has_desired_files, results))
    return [result for result, keep in zip(results, filtered) if keep]
...
if search_trigger:
    ...
    if st.session_state.use_ddg_search:
        with st.spinner(f"Searching DuckDuckGo for '{search_term}'..."):
            ddg_results = search_archive_with_duckduckgo(search_term, selected_media_type, start_year=start_year)
